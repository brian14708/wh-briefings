...
def parse_index(html: str) -> Tuple[List[Article], Optional[str]]:
    ...
    for a in soup.find("main").find_all("li", class_="wp-block-post"):
        ...
        *_, category = (
            urllib.parse.urlparse(a.find("a", rel="tag")["href"])
            .path.rstrip("/")
            .split("/")
        )
        *_, slug = urllib.parse.urlparse(link).path.rstrip("/").split("/")
        articles.append(
            Article(
...
async def parse_article(html: str, article: Article) -> str:
    ...
    if mt:
        article.modified_time = datetime.fromisoformat(mt["content"]).astimezone(
            article.published_time.tzinfo
        )
    main = soup.find("main")
...
async def fetch_index_worker(
    session: aiohttp.ClientSession, queue: asyncio.Queue[Article], oldest: datetime
) -> List[Article]:
...
async def fetch_article_worker(
    session: aiohttp.ClientSession, queue: asyncio.Queue[Article]
):
    while True:
        a = await queue.get()
        try:
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        tasks = [
            asyncio.create_task(fetch_article_worker(session, queue)) for _ in range(4)
        ]
    ...
    if len(prev_items) != len(items):
        ...
        async with aiofiles.open("rss.xml", "w") as f:
            feed = feedendum.feed.Feed(
                ...
                update=max(map(lambda i: i.update, items), default=datetime.now(UTC)),
                items=items,
            )
